...
| [ ] Fix `testpaths` so per-plugin test suites are collected | above | `pyproject.toml` sets `testpaths = ["tests"]`, so the 2 tests in `plugins/echo_plugin/tests/` are collected by neither a bare `pytest` nor CI. **Confirmed in CI**, not just locally: the first run logged `collected 13 items`, not 15. They pass when run explicitly. Every real connector inherits this blind spot, so fix before Stage 2. Either add `plugins` to `testpaths` or add a per-plugin pytest step to CI -- touches core config, so decide deliberately |
